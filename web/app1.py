...
# ============================================================================
# LANGGRAPH AUTONOMOUS PLANNER ROUTES (NEW)
# ============================================================================

@app.route('/lgraph/planner')
@login_required
...
@app.route('/api/lgraph/planner/chat', methods=['POST'])
...
@app.route('/lgraph/create-plan')
@login_required
def create_autonomous_plan_page():
    """Display the create autonomous plan form (NEW PAGE)"""
    user = user_registry.get_user(session['user_id'])
    return render_template('create_autonomous_plan.html', user=user)


@app.route('/api/lgraph/create-plan', methods=['POST'])
def api_lgraph_create_plan():
    """Create a new autonomous plan using LangGraph (NEW API)"""
    try:
        data = request.get_json()
...
@app.route('/lgraph/plan/<plan_id>')
@login_required
def lgraph_plan_detail(plan_id):
    """Display details of a specific LangGraph plan (NEW PAGE)"""
    user = user_registry.get_user(session['user_id'])
    user_id = session.get('user_id', 'demo_user')
...
@app.route('/api/lgraph/plan/<plan_id>/approve', methods=['POST'])
...
@app.route('/api/lgraph/plan/<plan_id>/reject', methods=['POST'])
...
@app.route('/api/lgraph/plan/<plan_id>/execute', methods=['POST'])
